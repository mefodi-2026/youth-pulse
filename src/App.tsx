...
import { createSession, ensureAuth, firebaseReady, joinSession, markPersonalViewed, saveAnswer, subscribeSession, updatePhase } from './lib/firebase'
import { recommendation, scoreAnswers } from './lib/scoring'
import { StageDashboard } from './StageDashboard'
import { MobileParticipantFlow } from './MobileParticipantFlow'
import type { Answer, Participant, Scores, Session, SessionPhase } from './types'
...
function Host() {
  ...
  const create = async () => {
    ...
    try {
      if (firebaseReady) { const user = await ensureAuth(); if (!user) throw new Error('Не удалось войти в Firebase'); await createSession(newRoom, user.uid) }
      else setDemo({ roomId: newRoom, createdAt: Date.now(), phase: 'lobby', maxParticipants: 30, hostUid: 'demo-host', participants: {} })
  ...
  const changePhase = async (next: SessionPhase) => {
    if (!session) return
    if (firebaseReady) await updatePhase(room, next)
    else { const nextSession = { ...session, phase: next, ...(next === 'resultsIntro' ? { resultsIntroStartedAt: Date.now() } : {}) }; setDemo(nextSession); setSession(nextSession) }
  }
  const start = () => {
  ...
  return <main className="host-shell"><aside className="host-menu"><div className="brand"><span>✦</span><b>Атмосфера</b><small>панель ведущего</small></div><nav>{menu.map(([id, label, icon]) => <button key={id} className={tab === id ? 'selected' : ''} onClick={() => setTab(id)}><span>{icon}</span>{label}</button>)}</nav><div className="menu-room"><small>АКТИВНАЯ КОМНАТА</small><b>{room}</b><span>{participants.length} из {session.maxParticipants} участников</span></div></aside><section className="host-content"><header className="host-header"><div><p className="eyebrow">СЕССИЯ · {room}</p><h1>{tab === 'overview' ? 'Управление сессией' : menu.find(item => item[0] === tab)?.[1]}</h1></div><span className={`status ${firebaseReady ? '' : 'demo'}`}>{firebaseReady ? 'ЭФИР АКТИВЕН' : 'ДЕМО-РЕЖИМ'}</span></header>
    ...
    {tab === 'rooms' && <div className="stack"><Glass className="room-row"><div><p className="eyebrow">ТЕКУЩАЯ</p><h2>{room}</h2><p>{phaseText(session.phase)} · {participants.length} подключились · {finished} завершили</p></div><Button onClick={() => void create()}>Создать новую комнату</Button></Glass><Glass className="empty-state"><h3>История комнат</h3><p>Сейчас показана активная комната. После запуска сессии её данные хранятся в Firebase и доступны из меню экспорта.</p></Glass></div>}
    {tab === 'questions' && <div className="question-admin">{Object.entries(categories).map(([id, title]) => <Glass key={id} className="question-group"><p className="eyebrow">4 ВОПРОСА</p><h3>{title}</h3>{questions.filter(question => question.category === id).map((question, index) => <div className="question-row" key={question.id}><span>{index + 1}</span>{question.title}</div>)}</Glass>)}</div>}
...
function exportCsv(session: Session) {
  const rows = Object.values(session.participants).map(participant => {
    const scores = scoreAnswers(participant.answers)
    return [participant.id, participant.nickname, participant.status, ...questions.flatMap(question => [question.title, participant.answers[question.id] ? question.options[participant.answers[question.id]] : '']), scores.total, ...Object.values(scores.categories)]
  })
...
function Stage({ room }: { room: string }) {
  ...
  const total = Math.max(people.length * questions.length, 1)
...
function Results({ room }: { room: string }) {
  ...
  const people = Object.values(session?.participants || {})
  const real = useMemo(() => { if (!people.length) return { communication: 84, forgiveness: 71, service: 79, care: 68 }; const values = people.map(person => scoreAnswers(person.answers || {}).categories); return Object.fromEntries(Object.keys(categories).map(key => [key, Math.round(values.reduce((sum, item) => sum + item[key as keyof typeof item], 0) / values.length)])) as Record<keyof typeof categories, number> }, [people])
