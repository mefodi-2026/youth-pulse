  ...
  return <main className="landing landing-product">
    ...
    <div className="landing-grid">
      <section className="landing-copy">
        ...
        <h1>Молодёжный Вайб<br />для вашей встречи</h1>
      ...
      <Glass className="landing-visual">
        ...
        <p className="eyebrow">МОЛОДЁЖНЫЙ ВАЙБ</p>
  ...
  return <main className="account-loading-screen" role="status" aria-live="polite" aria-busy={!waitingForAssets}>
    <div className="account-loading-mark" aria-hidden="true">
      <svg className="account-loading-arcs" viewBox="0 0 160 160" fill="none">
        ...
        <circle className="account-loading-arc account-loading-arc-b" cx="80" cy="80" r="52" pathLength="100" />
        <circle className="account-loading-arc account-loading-arc-c" cx="80" cy="80" r="38" pathLength="100" />
      </svg>
...
const authErrorText = (error: unknown) => {
  ...
  if (code === 'auth/operation-not-allowed') return 'Вход по email и паролю пока недоступен. Обратитесь к владельцу платформы.'
...
const packLibraryErrorText = (error: unknown) => {
  ...
  if (code === 'PERMISSION_DENIED' || /permission_denied/i.test(error instanceof Error ? error.message : '')) {
    return 'Доступ к опубликованным наборам сейчас ограничен. Обратитесь к владельцу платформы.'
  }
...
function AuthPage({ mode }: { mode: 'login' | 'register' | 'owner-login' }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!firebaseReady) return setError('Вход сейчас недоступен. Попробуйте позже или обратитесь к владельцу платформы.')
...
function ProfilePanel({ profile }: { profile: LeaderProfile }) {
  const [workspace, setWorkspace] = useState<Workspace | null>(null)
  useEffect(() => subscribeWorkspace(profile.workspaceId, setWorkspace), [profile.workspaceId])
  return <div className="stack"><Glass className="profile-panel"><p className="eyebrow">ПРОФИЛЬ ВЕДУЩЕГО</p><h2>{profile.fullName}</h2><p>Данные вашей молодёжной команды, комнат и результатов надёжно разделены.</p><dl><div><dt>Молодёжка</dt><dd>{workspace?.name || profile.workspaceId}</dd></div>{workspace?.city && <div><dt>Город</dt><dd>{workspace.city}</dd></div>}<div><dt>Email</dt><dd>{profile.email}</dd></div><div><dt>Телефон</dt><dd>{profile.phone}</dd></div></dl><Button secondary onClick={() => void logoutLeader().then(() => go('/login'))}>Выйти из аккаунта</Button></Glass></div>
}
...
function Host({ leader, initialTab, initialRoom }: { leader: LeaderProfile; initialTab?: HostTab; initialRoom?: string }) {
  ...
  const create = async (title = roomTitleDraft) => {
    ...
    try {
      ...
      if (!activePack && systemPacksState === 'error') throw new Error(`Не удалось загрузить опубликованный набор: ${systemPacksError || 'проверьте подключение к сервису.'}`)
  ...
  const addQuizPack = async (pack: ContentPack) => {
    if (workspaceQuizPacks[pack.packId]) {
      setQuizPackActions(previous => ({ ...previous, [pack.packId]: { state: 'existing', message: 'Набор уже есть в вашей библиотеке.' } }))
      return
    }
    ...
    try {
      ...
      setQuizPackActions(previous => ({ ...previous, [pack.packId]: {
        ...
        message: copied.outcome === 'copied' ? 'Набор добавлен в библиотеку.' : 'Набор уже есть в вашей библиотеке.',
      } }))
      setTemplateSelection(workspaceQuizSelection(copied.pack.packId))
    } catch (error) {
      setQuizPackActions(previous => ({ ...previous, [pack.packId]: { state: 'error', message: error instanceof Error ? error.message : 'Не удалось добавить набор викторины в библиотеку.' } }))
  ...
  const packSelectionControl = <section className="room-create-packs">
    ...
    {systemPacksState === 'loading'
      ...
      : systemPacksState === 'error'
        ? <><h3>Библиотека недоступна</h3><p className="connection-warning">{systemPacksError || 'Не удалось получить набор. Проверьте подключение и попробуйте снова.'}</p></>
  ...
  if (tab === 'quiz') return <HostLayout menu={menu} tab={tab} onTab={navigate} room={room} session={session} participants={participants.length} menuOpen={menuOpen} setMenuOpen={setMenuOpen}>
    <header className="host-header quiz-pilot-header"><div><p className="eyebrow">РЕЖИМ · ВИКТОРИНА</p><h1>{getModeDefinition(quizMode).title}</h1><p className="room-header-title">{getModeDefinition(quizMode).description}</p></div></header>
    <Glass className="mode-intro quiz-pilot-intro"><p className="eyebrow">НОВАЯ ВИКТОРИНА</p><h2>Выберите уровень и начните игру</h2><p>Викторина использует отдельные вопросы, правильные ответы, подсчёт баллов и топ-3. Наборы доступны для просмотра и запуска.</p><div className="control-actions"><Button className="quiz-pilot-primary-action" onClick={() => openRoomSetup(quizMode)}><span>Создать комнату викторины</span><AppIcon name="arrow-right" size={18} /></Button></div></Glass>
    <div className="quiz-pack-list quiz-pilot-pack-list">{quizSystemPacks.map(pack => { const copied = workspaceQuizPacks[pack.packId]; const action = quizPackActions[pack.packId]; const actionIsError = action?.state === 'error'; return <Glass className="quiz-pack-row quiz-pilot-pack-card" key={pack.packId}><div className="quiz-pilot-pack-copy"><p className="eyebrow">{pack.difficulty === 'easy' ? 'ЛЁГКИЙ УРОВЕНЬ' : pack.difficulty === 'medium' ? 'СРЕДНИЙ УРОВЕНЬ' : 'СЛОЖНЫЙ УРОВЕНЬ'}</p><h3>{displayPackTitle(pack.title)}</h3><p className="quiz-pilot-pack-meta">{pack.questions.length} вопросов · версия {pack.packVersion}</p>{action?.message && <p className={`quiz-pilot-feedback ${actionIsError ? 'is-error' : ''}`} role="status">{action.message}</p>}</div><div className="quiz-pilot-pack-cta">{copied ? <Button secondary className="quiz-pilot-workspace-action" onClick={() => setTemplateSelection(workspaceQuizSelection(pack.packId))}>{action?.state === 'copied' ? 'Добавлено в библиотеку' : 'Уже в библиотеке'}</Button> : <Button secondary className="quiz-pilot-workspace-action" disabled={action?.state === 'adding'} onClick={() => void addQuizPack(pack)}>{action?.state === 'adding' ? 'Добавляем…' : 'Добавить в библиотеку'}</Button>}</div><QuestionPackPreview appearance="quiz" showActionIcons modeLabel="Библейская викторина" packId={pack.packId} questions={pack.questions} /></Glass> })}</div>
  </HostLayout>
  if (tab === 'questions') return <HostLayout menu={menu} tab={tab} onTab={navigate} room={room} session={session} participants={participants.length} menuOpen={menuOpen} setMenuOpen={setMenuOpen}><header className="host-header"><div><p className="eyebrow">МАТЕРИАЛЫ · ПРОВЕРЬ СЕБЯ</p><h1>Вопросы</h1></div></header><QuestionPackPreview modeLabel="Проверь себя" packId={activeDiagnosticPack?.packId || diagnosticPackId} title={displayPackTitle(activeDiagnosticPack?.title)} questions={diagnosticQuestions} /></HostLayout>
  ...
  return <HostLayout menu={menu} tab={tab} onTab={navigate} room={room} session={session} participants={participants.length} menuOpen={menuOpen} setMenuOpen={setMenuOpen}>
    ...
    {tab === 'settings' && <div className="stack"><Glass className="settings-panel"><p className="eyebrow">ПОДКЛЮЧЕНИЕ ПО QR</p><h2>Адрес для участников</h2><p>Этот адрес используется в QR-коде и при копировании ссылки. Укажите адрес опубликованного сайта, если хотите заменить его.</p><input value={publicOrigin} onChange={event => setPublicOrigin(event.target.value)} placeholder="https://ваш-сайт.web.app" /><small>Подключение к сервису: {firebaseReady ? 'готово' : 'требует настройки'}</small></Glass><Glass className="settings-panel"><p className="eyebrow">ОБРАТНАЯ СВЯЗЬ ПОСЛЕ ВСТРЕЧИ</p><h2>Форма обратной связи</h2><p>Вставьте полную ссылку на форму. После завершения комнаты участники смогут открыть её по кнопке обратной связи.</p><input value={feedbackFormUrl} onChange={event => setFeedbackFormUrl(event.target.value)} placeholder="https://docs.google.com/forms/d/e/.../viewform" />{feedbackFormUrl && !feedbackUrl && <p className="connection-warning">Проверьте адрес формы: ссылка должна быть полной и начинаться с https://.</p>}</Glass><Glass className="settings-panel"><p className="eyebrow">СЕССИЯ</p><h2>Завершение</h2><p>После завершения участники больше не смогут отвечать. Участники, ответы, результаты и экспорт останутся в архиве.</p><Button secondary disabled={session.phase === 'closed'} onClick={closeRoom}>{session.phase === 'closed' ? 'Комната завершена' : 'Завершить комнату'}</Button>{actionError && <p className="connection-warning">{actionError}</p>}</Glass></div>}
  </HostLayout>
}
