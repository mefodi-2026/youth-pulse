...
type HostTab = 'main' | 'roomSetup' | 'overview' | 'currentRoom' | 'rooms' | 'results' | 'questions' | 'export' | 'settings' | 'profile' | 'rules'
type HostMenuItem = [HostTab, string, string]
const hostTabs: HostTab[] = ['main', 'roomSetup', 'overview', 'currentRoom', 'rooms', 'results', 'questions', 'export', 'settings', 'profile', 'rules']
...
function Host({ leader, initialTab, initialRoom }: { leader: LeaderProfile; initialTab?: HostTab; initialRoom?: string }) {
  ...
  if (!session && tab === 'questions') return <HostLayout menu={menu} tab={tab} onTab={navigate} room={lastClosedRoom} session={null} participants={0} menuOpen={menuOpen} setMenuOpen={setMenuOpen}>
    ...
    <Glass className="empty-state"><h3>{questionBank.length ? `Доступно вопросов: ${questionBank.length}` : 'Вопросник пока недоступен'}</h3><p>{questionBank.length ? 'Откройте настройку комнаты, чтобы использовать опубликованный набор. Личные изменения сохраняются в рабочем пространстве ведущего.' : 'Владелец платформы должен добавить и опубликовать системный набор в глобальной библиотеке.'}</p><Button onClick={openRoomSetup}>Перейти к созданию комнаты</Button></Glass>
  </HostLayout>
