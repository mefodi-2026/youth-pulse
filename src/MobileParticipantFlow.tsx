...
export function MobileParticipantFlow({ room }: { room: string }) {
  ...
  if (!activeQuestions.length) return <Shell screen="waiting-screen"><p className="flow-label">НАБОР БЕЗ ВОПРОСОВ</p><h1>Диагностика пока недоступна</h1><p>Ведущий выбрал набор без вопросов. Попросите его выбрать другой материал и создать новую комнату.</p></Shell>
