...
function PersonalReport({ participant, scores, onClose }: { participant: Participant; scores: Scores; onClose: () => void }) {
  ...
  return <Shell screen="report-screen"><p className="flow-label gold">ТВОЯ КАРТОЧКА</p><h1>{participant.nickname}</h1><div className="report-summary"><ScoreRing score={scores.total} /><p>Тёплая, живая атмосфера<br />с пространством<br />для роста</p></div><h2>Твои показатели</h2><div className="report-bars">{Object.entries(scores.categories).map(([id, value], index) => <div key={id} style={{ transitionDelay: `${index * 110}ms` }}><span>{categories[id as keyof typeof categories]}</span><b>{value}%</b><i><em style={{ width: showBars ? `${value}%` : '0%' }} /></i></div>)}</div><div className="wish-card"><small>Пожелание</small><p>{recommendation(scores)}</p></div><div className="report-downloads"><Action secondary onClick={() => window.print()}>Скачать PDF</Action><Action secondary onClick={() => createPoster(participant, scores)}>Скачать PNG</Action></div><button className="return-link" onClick={onClose}>Вернуться к ожиданию <span>→</span></button></Shell>
}
...
export function MobileParticipantFlow({ room }: { room: string }) {
  ...
  if (!participant && screen === 'intro') return <Shell screen="intro-screen"><p className="flow-label gold">ОНЛАЙН-ДИАГНОСТИКА</p><h1>Атмосфера<br />нашей молодёжи</h1><p>Небольшая анонимная диагностика, которая помогает увидеть сильные стороны и точки роста.</p><div className="intro-info"><b>✦</b><strong>16 простых вопросов</strong><small>4 темы · около 4 минут · без оценок</small><i /><span>В конце ты получишь личную карточку с результатами.</span></div><Action onClick={() => setScreen('nickname')}>Начать диагностику</Action><small className="flow-footnote">Твоя искренность поможет нам стать ближе.</small></Shell>
  ...
  if (participant.status === 'finished') return <PersonalReport participant={participant} scores={scoreAnswers(participant.answers || {})} onClose={() => setShowReport(false)} />
  const question = questions[participant.currentQuestionIndex]
  const done = Math.round(participant.currentQuestionIndex / questions.length * 100)
  return <Shell screen="question-screen"><div className="question-top"><div><span>ВОПРОС {participant.currentQuestionIndex + 1} / {questions.length}</span><small>{categories[question.category]}</small></div><b>{done}%</b></div><div className="question-progress"><i style={{ width: `${done}%` }} /></div><h1>{question.title}</h1><p>Выбери вариант, который ближе всего к тебе.</p><div className="answer-options">{(['A', 'B', 'C', 'D'] as Answer[]).map(letter => <button disabled={saving} key={letter} onClick={() => void answer(letter)}><b>{letter}</b><span>{question.options[letter]}</span></button>)}</div><small className="skip-note">Отвечай так, как чувствуешь — здесь нет правильных ответов.</small></Shell>
}
