...
import type { Answer, CategoryId, Scores } from '../types'

const weight: Record<Answer, number> = { A: 3, B: 2, C: 1, D: 0 }
export const scoreAnswers = (answers: Record<string, Answer> = {}): Scores => {
  ...
  for (const category of Object.keys(categories) as CategoryId[]) {
    ...
    result[category] = Math.round(items.reduce((sum, q) => sum + (answers[q.id] ? weight[answers[q.id]] : 0), 0) / (items.length * 3) * 100)
  }
  return { categories: result, total: Math.round(Object.values(result).reduce((sum, value) => sum + value, 0) / 4) }
}
