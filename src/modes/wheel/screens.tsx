...
function FortuneWheel({ spin, items = [], spinning, target, completed = false }: { spin?: WheelSpinAnimation; items?: WheelSpinItem[]; spinning: boolean; target?: 'name' | 'task' | null; completed?: boolean }) {
  ...
  if (!sectors.length) return <div className={`fortune-wheel-empty${completed ? ' is-completed' : ''}`}><span>◉</span><p>{completed ? 'Все новые пары разыграны' : 'Добавьте элементы, чтобы собрать колесо'}</p></div>
...
type AudienceProps = { phase: keyof typeof phaseLabels; drawOrder: WheelDrawOrder; round?: WheelPublicRound; activeSpin?: WheelSpinAnimation; nameCount: number; taskCount: number; roundCount: number; pendingCount: number; idleItems?: WheelSpinItem[]; stopAnimation?: boolean; showReveal?: boolean; onOpenLibrary?: () => void }

function WheelAudiencePanel({ phase, drawOrder, round, activeSpin, nameCount, taskCount, roundCount, pendingCount, idleItems = [], stopAnimation = false, showReveal = true, onOpenLibrary }: AudienceProps) {
  ...
  const queueEmptyCopy = phase === 'completed' ? 'Новых вариантов для колеса больше нет.' : 'Список появится после начала игры.'
  return <section className={`wheel-audience ${phase === 'performing' ? 'wheel-audience-performing' : ''}`}>
    ...
    <div className="wheel-play-layout"><aside className="wheel-active-list"><small>{queueLabel}</small>{items.length ? <ol>{items.map((item, index) => <li key={item.itemId}><b>{index + 1}</b><span>{item.text}</span></li>)}</ol> : <p>{queueEmptyCopy}</p>}<footer>{items.length} {target === 'task' ? 'заданий' : target === 'name' ? 'имён в колесе' : 'элементов в колесе'}</footer></aside><div className="wheel-visual">{phase === 'performing' ? <article className="wheel-performing-card"><small>РАУНД {roundCount + 1}</small><p>Задание выполняет <b>{round?.selectedNameText || 'участник'}</b></p><strong>{round?.selectedTaskText || '—'}</strong><span>● Раунд начат</span></article> : <><FortuneWheel spin={activeSpin} items={items} spinning={spinning} target={target} completed={phase === 'completed'} />{spinning && <p className="wheel-spinning-copy">Колесо вращается · повторный запуск заблокирован</p>}{showReveal && selectedTitle && <article className="wheel-reveal-card"><small>{selectedTitle}</small><strong>{phase === 'name_revealed' ? round?.selectedNameText : round?.selectedTaskText}</strong><span>Подтвердите выбор, чтобы продолжить.</span></article>}</>}</div><aside className="wheel-round-card"><small>{phase === 'completed' ? 'ИТОГ ИГРЫ' : 'ТЕКУЩИЙ РАУНД'}</small><div><span>ИМЯ</span><strong>{round?.selectedNameText || '—'}</strong></div><div><span>ЗАДАНИЕ</span><strong>{round?.selectedTaskText || '—'}</strong></div>{pendingCount > 0 && onOpenLibrary ? <button type="button" className="wheel-pending-shortcut" onClick={onOpenLibrary}>Отложенные задания <b>{pendingCount}</b></button> : <p>{phase === 'completed' ? 'Игра завершена.' : phase === 'performing' ? 'Задание выполняется сейчас.' : 'Ожидаем следующий выбор.'}</p>}</aside></div>
  </section>
}

function WheelHistory({ history }: { history: WheelPublicHistoryItem[] }) {
  if (!history.length) return null
  return <details className="wheel-history"><summary><span><p className="eyebrow">ИСТОРИЯ РАУНДОВ</p><strong>Сыгранные пары</strong></span><b>{history.length}</b></summary><ol>{history.map((item, index) => <li key={item.roundId}><b>{index + 1}</b><span><strong>{item.nameText}</strong>{item.taskText}</span><em>{item.status === 'pending' ? 'В библиотеке' : 'Выполнено'}</em></li>)}</ol></details>
}
...
export function WheelParticipantFlow({ room }: ModeParticipantFlowProps) {
  ...
  if (publicWheel.phase !== 'collecting') return <main className="mobile-wrap wheel-mobile"><section className="mobile-card"><WheelAudiencePanel phase={publicWheel.phase} drawOrder={publicWheel.drawOrder} round={publicWheel.currentRound} activeSpin={publicWheel.activeSpin} nameCount={publicWheel.nameCount} taskCount={publicWheel.taskCount} roundCount={publicWheel.roundCount ?? 0} pendingCount={publicWheel.pendingCount ?? 0} />{error && <p className="flow-error">{error}</p>}</section></main>
...
export function WheelHostScreen({ session, joinUrl, onClose, onPlayAgain, onExitToMain }: ModeHostScreenProps) {
  ...
  return <div className="wheel-host">
    ...
    {collecting && <><section className="wheel-prep-heading"><div><h1>Подготовка комнаты</h1><p>{participantInput ? 'Участники добавляют имена и задания перед стартом' : 'Заполните оба списка перед стартом'}</p></div><div className="wheel-counters"><article><span>◉ Имен</span><strong>{getAvailableWheelCount(wheel, 'name')}</strong></article><article><span>▣ Заданий</span><strong>{getAvailableWheelCount(wheel, 'task')}</strong></article>{participantInput ? <article><span>♙ Подключились</span><strong>{Object.keys(entries).length}</strong></article> : <article><span>◉ Источник данных</span><strong className="wheel-source-label">Ведущий</strong></article>}</div></section>
      ...
      {wheel?.config.inputMode === 'host'
        ? <section className="wheel-prep-workspace"><div className="wheel-prep-column"><div className="wheel-prep-input"><label>Введите имя участника<input value={name} onChange={event => setName(event.target.value)} maxLength={60} /></label><button type="button" className="button secondary" disabled={busy || !name.trim() || Object.keys(names).length >= 50} onClick={() => add('names')}>＋ Добавить имя</button></div><WheelList title="Имена в колесе" items={names} onDelete={id => removeItem('names', id)} onClear={() => clearItems('names')} /></div><div className="wheel-prep-column"><div className="wheel-prep-input"><label>Введите задание<textarea value={task} onChange={event => setTask(event.target.value)} maxLength={240} /></label><button type="button" className="button secondary" disabled={busy || !task.trim() || Object.keys(tasks).length >= 50} onClick={() => add('tasks')}>＋ Добавить задание</button></div><WheelList title="Задания" items={tasks} onDelete={id => removeItem('tasks', id)} onClear={() => clearItems('tasks')} /></div></section>
        : <div className="wheel-pools"><WheelList title="Имена в колесе" items={names} /><WheelList title="Задания" items={tasks} /></div>}
      <section className="wheel-prep-actions"><button type="button" className="button" disabled={disabled || !valid} onClick={() => void run(() => markWheelReady(session.roomId))}>▶ Начать игру</button><p className="wheel-ready-note">{valid ? 'Списки готовы. После старта их нельзя будет изменить.' : `Нужны одинаковые списки от 2 до 50: сейчас ${Object.keys(names).length} / ${Object.keys(tasks).length}.`}</p></section></>}
    {!collecting && wheel && <section className={`glass wheel-game-panel ${performing ? 'wheel-game-panel-performing' : ''}`}>
      {performing && wheel.currentRound
        ...
        : <><WheelAudiencePanel phase={wheel.phase} drawOrder={wheel.config.drawOrder} round={fullVisibleRound(wheel)} activeSpin={ending ? undefined : wheel.activeSpin || undefined} nameCount={getAvailableWheelCount(wheel, 'name')} taskCount={getAvailableWheelCount(wheel, 'task')} roundCount={history.length} pendingCount={pending.length} idleItems={activeRoundItems} stopAnimation={ending} showReveal={false} onOpenLibrary={() => setLibraryOpen(true)} /><div className="wheel-game-actions">
    ...
    {wheel?.phase === 'completed' && <section className="glass wheel-finish-screen"><p className="eyebrow">ИГРА ЗАВЕРШЕНА</p><h2>{pending.length ? 'Новые пары закончились' : 'Все пары разыграны'}</h2><p>{pending.length ? `В библиотеке осталось заданий: ${pending.length}. Откройте их, чтобы завершить все начатые раунды.` : 'История этой игры сохранится в архиве. Для нового состава создайте отдельную игровую сессию.'}</p><div><button type="button" className="button" disabled={disabled} onClick={playAgain}>Сыграть ещё раз</button><button type="button" className="button secondary" disabled={disabled} onClick={() => setFinishIntent('exit')}>Выйти в главное меню</button></div></section>}
    {history.length > 0 && <WheelHistory history={history} />}
