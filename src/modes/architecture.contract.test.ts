...
const verseMatch = getModeManifest('verse-match')

assert(productionModes.length === 4, 'production registry must contain all four released modes')
...
assert(verseMatch.id === 'verse-match', 'verse-match must resolve through the registry')
