...
try {
  ...
  assert.match(verseFunctionsSource, /sign_in_provider === 'anonymous'/)
  assert.match(verseFunctionsSource, /game\.hostUid !== leader\.uid/)
  assert.match(verseFunctionsSource, /verseMatchParticipantViews\/\$\{game\.roomId\}\/\$\{participant\.id\}/)
  assert.doesNotMatch(verseFunctionsSource, /verseMatchParticipantViews.*packSnapshot/)
